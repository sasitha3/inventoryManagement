...
IS
BEGIN
   Error_SYS.Too_Many_Rows(Exm_Inventory_API.lu_name_, NULL, methodname_);
...
IS
BEGIN
   Error_SYS.Record_Not_Exist(Exm_Inventory_API.lu_name_);
...
PROCEDURE Raise_Record_Exist___ (
   rec_ exm_inventory_tab%ROWTYPE )
IS
BEGIN
   Error_SYS.Record_Exist(Exm_Inventory_API.lu_name_);
...
IS
BEGIN
   Error_SYS.Item_Format(Exm_Inventory_API.lu_name_, name_, value_);
...
IS
BEGIN
   Error_SYS.Record_Modified(Exm_Inventory_API.lu_name_);
...
IS
BEGIN
   Error_SYS.Record_Locked(Exm_Inventory_API.lu_name_);
...
IS
BEGIN
   Error_SYS.Record_Removed(Exm_Inventory_API.lu_name_);
...
FUNCTION Lock_By_Id___ (
   objid_      IN VARCHAR2,
   objversion_ IN VARCHAR2 ) RETURN exm_inventory_tab%ROWTYPE
IS
...
FUNCTION Lock_By_Keys___ (
   inventory_id_ IN NUMBER) RETURN exm_inventory_tab%ROWTYPE
IS
   row_locked  EXCEPTION;
   PRAGMA      EXCEPTION_INIT(row_locked, -0054);
   rec_        exm_inventory_tab%ROWTYPE;
BEGIN
   ...
   EXCEPTION
      WHEN no_data_found THEN
         Raise_Record_Removed___(inventory_id_);
      WHEN too_many_rows THEN
         Raise_Too_Many_Rows___(inventory_id_, 'Lock_By_Keys___');
   END;
END Lock_By_Keys___;
...
FUNCTION Get_Object_By_Id___ (
   objid_ IN VARCHAR2 ) RETURN exm_inventory_tab%ROWTYPE
IS
   lu_rec_ exm_inventory_tab%ROWTYPE;
BEGIN
   SELECT *
      INTO  lu_rec_
      FROM  exm_inventory_tab
...
EXCEPTION
   WHEN no_data_found THEN
      RETURN lu_rec_;
   WHEN too_many_rows THEN
      Raise_Too_Many_Rows___(inventory_id_, 'Get_Object_By_Keys___');
...
EXCEPTION
   WHEN no_data_found THEN
      RETURN FALSE;
   WHEN too_many_rows THEN
      Raise_Too_Many_Rows___(inventory_id_, 'Check_Exist___');
END Check_Exist___;
...
EXCEPTION
   WHEN no_data_found THEN
      ...
      objversion_ := NULL;
   WHEN too_many_rows THEN
      Raise_Too_Many_Rows___(inventory_id_, 'Get_Id_Version_By_Keys___');
...
FUNCTION Get_Key_By_Rowkey (
   rowkey_ IN VARCHAR2 ) RETURN exm_inventory_tab%ROWTYPE
IS
   rec_ exm_inventory_tab%ROWTYPE;
BEGIN
   SELECT inventory_id
      INTO  rec_.inventory_id
      FROM  exm_inventory_tab
...
EXCEPTION
   WHEN no_data_found THEN
      RETURN rec_;
   WHEN too_many_rows THEN
      Raise_Too_Many_Rows___(rec_.inventory_id, 'Get_Key_By_Rowkey');
...
FUNCTION Pack___ (
   rec_ IN exm_inventory_tab%ROWTYPE ) RETURN VARCHAR2
IS
...
FUNCTION Pack___ (
   rec_ IN exm_inventory_tab%ROWTYPE,
...
FUNCTION Get_Indicator_Rec___ (
   rec_ IN exm_inventory_tab%ROWTYPE ) RETURN Indicator_Rec
IS
...
FUNCTION Get_Indicator_Rec___ (
   oldrec_ IN exm_inventory_tab%ROWTYPE,
   newrec_ IN exm_inventory_tab%ROWTYPE ) RETURN Indicator_Rec
IS
   indrec_ Indicator_Rec;
BEGIN
...
PROCEDURE Check_Insert___ (
   ...
   attr_   IN OUT VARCHAR2 )
IS
   oldrec_ exm_inventory_tab%ROWTYPE;
BEGIN
...
EXCEPTION
   WHEN dup_val_on_index THEN
      ...
      BEGIN
         IF (constraint_ = 'EXM_INVENTORY_RK') THEN
...
IS
   ...
   attr_          VARCHAR2(32000);
   indrec_        Indicator_Rec;
   emptyrec_      exm_inventory_tab%ROWTYPE;
BEGIN
...
EXCEPTION
   WHEN dup_val_on_index THEN
      ...
      BEGIN
         IF (constraint_ = 'EXM_INVENTORY_RK') THEN
            Error_SYS.Rowkey_Exist(Exm_Inventory_API.lu_name_, newrec_.rowkey);
         ELSE
            Raise_Record_Exist___(newrec_);
         END IF;
      END;
...
BEGIN
   IF (lock_mode_wait_) THEN
      oldrec_ := Lock_By_Keys___(newrec_.inventory_id);
   ELSE
      oldrec_ := Lock_By_Keys_Nowait___(newrec_.inventory_id);
   END IF;
...
PROCEDURE Check_Delete___ (
   remrec_ IN exm_inventory_tab%ROWTYPE )
IS
   key_ VARCHAR2(2000);
BEGIN
   key_ := remrec_.inventory_id||'^';
...
PROCEDURE Remove___ (
   remrec_         IN OUT exm_inventory_tab%ROWTYPE,
   lock_mode_wait_ IN     BOOLEAN DEFAULT TRUE )
IS
   oldrec_     exm_inventory_tab%ROWTYPE;
BEGIN
   IF (lock_mode_wait_) THEN
      oldrec_ := Lock_By_Keys___(remrec_.inventory_id);
   ELSE
      oldrec_ := Lock_By_Keys_Nowait___(remrec_.inventory_id);
   END IF;
...
IS
   dummy_ exm_inventory_tab%ROWTYPE;
BEGIN
...
PROCEDURE New__ (
   ...
   action_     IN     VARCHAR2 )
IS
   newrec_   exm_inventory_tab%ROWTYPE;
   indrec_   Indicator_Rec;
BEGIN
...
PROCEDURE Remove__ (
   ...
   action_     IN  VARCHAR2 )
IS
   remrec_ exm_inventory_tab%ROWTYPE;
BEGIN
   IF (action_ = 'CHECK') THEN
...
IS
BEGIN
   IF (NOT Check_Exist___(inventory_id_)) THEN
      Raise_Record_Not_Exist___(inventory_id_);
   END IF;
END Exist;
...
IS
   rowkey_ exm_inventory_tab.rowkey%TYPE;
BEGIN
...
EXCEPTION
   WHEN no_data_found THEN
      RETURN NULL;
   WHEN too_many_rows THEN
      Raise_Too_Many_Rows___(inventory_id_, 'Get_Objkey');
END Get_Objkey;
