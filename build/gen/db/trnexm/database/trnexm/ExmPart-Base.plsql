...
IS
BEGIN
   Error_SYS.Too_Many_Rows(Exm_Part_API.lu_name_, NULL, methodname_);
...
IS
BEGIN
   Error_SYS.Record_Not_Exist(Exm_Part_API.lu_name_);
...
PROCEDURE Raise_Record_Exist___ (
   rec_ exm_part_tab%ROWTYPE )
IS
BEGIN
   Error_SYS.Record_Exist(Exm_Part_API.lu_name_);
...
IS
BEGIN
   Error_SYS.Item_Format(Exm_Part_API.lu_name_, name_, value_);
...
IS
BEGIN
   Error_SYS.Record_Modified(Exm_Part_API.lu_name_);
...
IS
BEGIN
   Error_SYS.Record_Locked(Exm_Part_API.lu_name_);
...
IS
BEGIN
   Error_SYS.Record_Removed(Exm_Part_API.lu_name_);
...
FUNCTION Lock_By_Id___ (
   objid_      IN VARCHAR2,
   objversion_ IN VARCHAR2 ) RETURN exm_part_tab%ROWTYPE
IS
...
FUNCTION Lock_By_Keys___ (
   part_no_ IN NUMBER) RETURN exm_part_tab%ROWTYPE
IS
   row_locked  EXCEPTION;
   PRAGMA      EXCEPTION_INIT(row_locked, -0054);
   rec_        exm_part_tab%ROWTYPE;
BEGIN
   ...
   EXCEPTION
      WHEN no_data_found THEN
         Raise_Record_Removed___(part_no_);
      WHEN too_many_rows THEN
         Raise_Too_Many_Rows___(part_no_, 'Lock_By_Keys___');
   END;
END Lock_By_Keys___;
...
FUNCTION Get_Object_By_Id___ (
   objid_ IN VARCHAR2 ) RETURN exm_part_tab%ROWTYPE
IS
   lu_rec_ exm_part_tab%ROWTYPE;
BEGIN
   SELECT *
      INTO  lu_rec_
      FROM  exm_part_tab
...
EXCEPTION
   WHEN no_data_found THEN
      RETURN lu_rec_;
   WHEN too_many_rows THEN
      Raise_Too_Many_Rows___(part_no_, 'Get_Object_By_Keys___');
...
EXCEPTION
   WHEN no_data_found THEN
      RETURN FALSE;
   WHEN too_many_rows THEN
      Raise_Too_Many_Rows___(part_no_, 'Check_Exist___');
END Check_Exist___;
...
EXCEPTION
   WHEN no_data_found THEN
      ...
      objversion_ := NULL;
   WHEN too_many_rows THEN
      Raise_Too_Many_Rows___(part_no_, 'Get_Id_Version_By_Keys___');
...
EXCEPTION
   WHEN no_data_found THEN
      RETURN rec_;
   WHEN too_many_rows THEN
      Raise_Too_Many_Rows___(rec_.part_no, 'Get_Key_By_Rowkey');
...
PROCEDURE Unpack___ (
   newrec_   IN OUT exm_part_tab%ROWTYPE,
...
FUNCTION Pack___ (
   rec_ IN exm_part_tab%ROWTYPE ) RETURN VARCHAR2
IS
...
FUNCTION Get_Indicator_Rec___ (
   rec_ IN exm_part_tab%ROWTYPE ) RETURN Indicator_Rec
IS
...
FUNCTION Get_Indicator_Rec___ (
   oldrec_ IN exm_part_tab%ROWTYPE,
   newrec_ IN exm_part_tab%ROWTYPE ) RETURN Indicator_Rec
IS
   indrec_ Indicator_Rec;
BEGIN
...
EXCEPTION
   WHEN dup_val_on_index THEN
      ...
      BEGIN
         IF (constraint_ = 'EXM_PART_RK') THEN
...
IS
   ...
   attr_          VARCHAR2(32000);
   indrec_        Indicator_Rec;
   emptyrec_      exm_part_tab%ROWTYPE;
BEGIN
...
EXCEPTION
   WHEN dup_val_on_index THEN
      ...
      BEGIN
         IF (constraint_ = 'EXM_PART_RK') THEN
            Error_SYS.Rowkey_Exist(Exm_Part_API.lu_name_, newrec_.rowkey);
         ELSE
            Raise_Record_Exist___(newrec_);
         END IF;
      END;
...
BEGIN
   IF (lock_mode_wait_) THEN
      oldrec_ := Lock_By_Keys___(newrec_.part_no);
   ELSE
      oldrec_ := Lock_By_Keys_Nowait___(newrec_.part_no);
   END IF;
...
PROCEDURE Check_Delete___ (
   remrec_ IN exm_part_tab%ROWTYPE )
IS
   key_ VARCHAR2(2000);
BEGIN
...
PROCEDURE Remove___ (
   remrec_         IN OUT exm_part_tab%ROWTYPE,
   lock_mode_wait_ IN     BOOLEAN DEFAULT TRUE )
IS
   oldrec_     exm_part_tab%ROWTYPE;
BEGIN
   IF (lock_mode_wait_) THEN
      oldrec_ := Lock_By_Keys___(remrec_.part_no);
   ELSE
      oldrec_ := Lock_By_Keys_Nowait___(remrec_.part_no);
   END IF;
...
BEGIN
   IF (NOT Check_Exist___(part_no_)) THEN
      Raise_Record_Not_Exist___(part_no_);
   END IF;
END Exist;
...
IS
BEGIN
   RETURN Check_Exist___(part_no_);
END Exists;
...
IS
   temp_ exm_part_tab.part_descrption%TYPE;
BEGIN
   IF (part_no_ IS NULL) THEN
      RETURN NULL;
   END IF;
   SELECT part_descrption
      INTO  temp_
      FROM  exm_part_tab
      WHERE part_no = part_no_;
   RETURN temp_;
EXCEPTION
   WHEN no_data_found THEN
      RETURN NULL;
   WHEN too_many_rows THEN
      Raise_Too_Many_Rows___(part_no_, 'Get_Part_Descrption');
...
EXCEPTION
   WHEN no_data_found THEN
      RETURN NULL;
   WHEN too_many_rows THEN
      Raise_Too_Many_Rows___(part_no_, 'Get');
END Get;
...
FUNCTION Get_Objkey (
   part_no_ IN NUMBER ) RETURN VARCHAR2
IS
   rowkey_ exm_part_tab.rowkey%TYPE;
BEGIN
   IF (part_no_ IS NULL) THEN
      RETURN NULL;
   END IF;
...
EXCEPTION
   WHEN no_data_found THEN
      RETURN NULL;
   WHEN too_many_rows THEN
      Raise_Too_Many_Rows___(part_no_, 'Get_Objkey');
END Get_Objkey;
